  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $('#tabelaInfo').append(`
        ...
        <th> Responsável </th>
        <th> C/ Bolsa</th>
        <th> ID Projeto </th> <!-- Nova coluna para ID_PROJETO -->
        <th> Opções </th>
      </tr>
      `);
      ...
      if (data.length > 0) {
        $.each(data, function (index, item) {
          ...
          // Adicionando o ID_PROJETO visível na tabela
          $("#linhaTabela").append(`
            <tr>
              <td>
                <div class="form-check form-check-muted m-0">
              ...
              <td>${item.NOME}</td>
              <td>${possui}</td>
              <td>${item.ID_PROJETO}</td> <!-- Exibindo o ID_PROJETO na nova coluna -->
              <td>
                <div class="action-buttons">
                  ...
                  <button title="Editar" onclick="openEditModal('${item.TITULO}', '${item.NOME_TIPO_PROJETO}', '${possui}', '${item.RESUMO}', '${item.CRITERIOS}', '${item.DESCRICAO}', '${item.REQUISITOS}')">
